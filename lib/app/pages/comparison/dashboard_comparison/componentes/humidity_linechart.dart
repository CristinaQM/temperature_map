...
class MyLineChart extends StatelessWidget {
  ...
  double get routesMinHum {
    ...
    maxHum = (maxHum / 5).floor().toDouble() * 5;
  ...
  List<FlSpot> myFlSpotList(Map<String, dynamic> ruta) {
    ...
    for (var dataPoint in ruta['dataList']) {
      myList.add(FlSpot(dataPoint['id'], dataPoint['humedad']));
    }
...
class MyHumidityLineChart extends StatefulWidget {
  final double maxWidth;
  const MyHumidityLineChart({super.key, required this.maxWidth});

  @override
  State<StatefulWidget> createState() => MyHumidityLineChartState();
}

class MyHumidityLineChartState extends State<MyHumidityLineChart> {
  @override
  Widget build(BuildContext context) {
    return Column(
